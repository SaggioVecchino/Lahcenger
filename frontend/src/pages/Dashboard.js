...
export default function Dashboard() {
  ...
  const handleRespondedSentRequestEvent = (payload, action) => {
  ...
  const handleCanceledRequestEvent = (payload) => {
    const { request_id } = payload;
    setRequests((prev) => prev.filter((req) => req.request_id !== request_id));
  };

  const handleReceivedRequestEvent = (payload) => {
